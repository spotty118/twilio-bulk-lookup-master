ActiveAdmin.register Contact do
  ...
  filter :device_type, as: :select, collection: %w[mobile landline voip]
  filter :line_type, as: :select, collection: %w[mobile landline voip fixedVoip nonFixedVoip tollFree]
  filter :carrier_name
  ...
  filter :sms_pumping_risk_level, as: :select, collection: %w[low medium high], label: 'Fraud Risk'
  ...
  filter :caller_type, as: :select, collection: %w[business consumer]
  ...
  filter :business_employee_range, as: :select,
                                   collection: ['1-10', '11-50', '51-200', '201-500', '501-1000', '1001-5000', '5001-10000', '10000+'], label: 'Company Size'
  filter :business_revenue_range, as: :select,
                                  collection: ['$0-$1M', '$1M-$10M', '$10M-$50M', '$50M-$100M', '$100M-$500M', '$500M-$1B', '$1B+'], label: 'Revenue Range'
  ...
  filter :business_enriched, as: :select, collection: [['Enriched', true], ['Not Enriched', false]],
  ...
  filter :address_type, as: :select, collection: %w[residential apartment po_box business],
                        label: 'Address Type'
  filter :address_verified, as: :select, collection: [['Verified', true], ['Not Verified', false]],
                            label: 'Address Verified'
  filter :address_enriched, as: :select, collection: [['Enriched', true], ['Not Enriched', false]],
                            label: 'Address Status'

  # Verizon coverage filters
  filter :verizon_5g_home_available, as: :select, collection: [['Available', true], ['Not Available', false]],
                                     label: 'Verizon 5G Home'
  filter :verizon_lte_home_available, as: :select, collection: [['Available', true], ['Not Available', false]],
                                      label: 'Verizon LTE Home'
  filter :verizon_fios_available, as: :select, collection: [['Available', true], ['Not Available', false]],
                                  label: 'Verizon Fios'
  filter :verizon_coverage_checked, as: :select, collection: [['Checked', true], ['Not Checked', false]],
  ...
  index do
    ...
    column 'Business', :business_name do |contact|
      if contact.business?
        div do
          strong contact.business_name, style: 'display: block; color: #2c3e50;' if contact.business_name.present?
          if contact.business_employee_range.present?
            small contact.business_size_category, style: 'color: #7f8c8d; display: block;'
          end
          status_tag 'Not Enriched', class: 'warning', style: 'margin-top: 5px;' unless contact.business_enriched?
        end
      else
        span '—', class: 'empty'
      end
    ...
    column 'Quality', :data_quality_score do |contact|
      if contact.data_quality_score
        ...
        div do
          ...
          status_tag 'Duplicate', class: 'error', style: 'margin-top: 3px;' if contact.is_duplicate
        end
      else
        span '—', class: 'empty'
      end
  ...
  show do
    ...
    panel '🏢 Business Intelligence' do
      if contact.business?
        attributes_table_for contact do
          ...
          row 'Company Size' do |c|
            if c.business_employee_count
              "#{c.business_employee_count.to_s.reverse.gsub(/(\d{3})(?=\d)/,
                                                             '\\1,').reverse} employees (#{c.business_size_category})"
        ...
        para 'This contact is not identified as a business.',
      ...
      if contact.business? && contact.business_enriched?
        attributes_table_for contact do
          ...
          row 'Full Address' do |c|
            parts = [c.business_address, c.business_city, c.business_state, c.business_postal_code,
                     c.business_country].compact
            if parts.any?
              parts.join(', ')
            else
              '—'
        ...
        attributes_table_for contact do
          ...
          row :business_twitter_handle do |c|
            if c.business_twitter_handle
              link_to "@#{c.business_twitter_handle}", "https://twitter.com/#{c.business_twitter_handle}",
                      target: '_blank'
            else
              '—'
            end
          ...
          row 'Full Address' do |c|
            if c.has_full_address?
              div do
                ...
                status_tag 'Verified', class: 'ok', style: 'margin-left: 10px;' if c.address_verified
              end
            else
              '—'
            end
      ...
      if contact.consumer? && contact.has_full_address?
        attributes_table_for contact do
          ...
          row 'Available Products' do |c|
            if c.verizon_coverage_checked?
              div do
                status_tag 'Fios', class: 'ok', style: 'margin: 2px;' if c.verizon_fios_available
                status_tag '5G Home', class: 'ok', style: 'margin: 2px;' if c.verizon_5g_home_available
                status_tag 'LTE Home', class: 'warning', style: 'margin: 2px;' if c.verizon_lte_home_available
